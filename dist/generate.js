...
export function generate(requirementsDir, canonDir, prefix, name) {
    ...
    if (prevSnapshot) {
        ...
        // Write snapshot
        writeFileSync(join(snapshotsDir, `${timestamp}_snapshot.json`), JSON.stringify(newSnapshot, null, 2));
        // Write change file
        const changeContent = formatChangeFile(ticket, name, diff);
        writeFileSync(join(canonDir, 'changes', `${ticket}_${name}.md`), changeContent);
        // Update journal
    ...
    const allEntries = Object.entries(requirements).map(([file, req]) => Object.entries(req.sections).map(([section, items]) => ({ file, section, items }))).flat();
