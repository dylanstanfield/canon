...
import type { Snapshot, RequirementFile } from './types.js'
...
export interface DiffResult {
  added: DiffEntry[]
  modified: { file: string; section: string; old: string[]; new: string[] }[]
  removed: DiffEntry[]
}

export function diffSnapshots(prev: Snapshot, curr: Snapshot): DiffResult | null {
...
function diffFile(
  fileKey: string,
  prev: RequirementFile,
  curr: RequirementFile,
  ...
  for (const section of currSections) {
    if (!prevSections.has(section)) continue
...
export function formatChangeFile(ticket: string, name: string, diff: DiffResult): string {
  const lines: string[] = [`# ${ticket}: ${name}`, '']
