...
export function checkIntegrity(canonDir: string, _requirementsDir: string): CheckResult {
  const errors: string[] = []
  ...
  const snapshots: Snapshot[] = snapshotFiles.map(f =>
    JSON.parse(readFileSync(join(canonDir, 'snapshots', f), 'utf-8')),
  )
  ...
  if (snapshots.length > 0) {
    ...
    for (let i = 1; i < snapshots.length; i++) {
      if (snapshots[i].prevId !== snapshots[i - 1].id) {
        errors.push(
          `Broken chain: snapshot ${snapshots[i].id} has prevId ${snapshots[i].prevId}, ` +
