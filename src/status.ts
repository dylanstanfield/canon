...
export function getStatus(canonDir: string, requirementsDir: string): StatusResult {
  // Read journal
  let journal: Journal
  try {
  ...
  if (latestSnapshot) {
    ...
    if (diff) {
      ...
      for (const e of [...diff.added, ...diff.removed]) files.add(e.file)
      for (const e of diff.modified) files.add(e.file)
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (!status.initialized) {
    ...
    if (status.pendingChanges.length > 0) {
      ...
      for (const f of status.pendingChanges) console.log(`  - ${f}`)
    } else {
      console.log('\nNo pending changes.')
    }
  }
